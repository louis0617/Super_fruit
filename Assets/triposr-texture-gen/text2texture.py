...
def raycast_mesh(tmesh, camera_dist=2.8, rot_x_rad=0.0, rot_y_rad=0.0, fov_deg=30, size=512):
    if rot_x_rad or rot_y_rad:
        ...
        tmesh.rotate(rot, center=tmesh.get_center())

    scene = o3d.t.geometry.RaycastingScene()
    scene.add_triangles(tmesh)

    rays = scene.create_rays_pinhole(fov_deg=fov_deg, center=[0, 0, 0], eye=[0, 0, camera_dist], up=[0, -1, 0], width_px=size, height_px=size)

    result = scene.cast_rays(rays)

    np_result = {k: result[k].numpy() for k in ('primitive_ids', 'primitive_uvs', 'primitive_normals')}
...
def text2texture(mesh, desc, steps, depth_txt2img_path, img_model, device, out_path_base, size=512):
    ...
    raycast, raycast_slices = raycast_mesh_multi(tmesh, 2.8, [(0, i * math.pi / 2) for i in range(4)], crop=True, size=size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-model', help='SD 1.5-based model for texture image gen', default='Lykon/dreamshaper-8')
    parser.add_argument('--steps', type=int, default=12, help='Num inference steps for texture image gen')
    parser.add_argument('--device', default='', type=str, help='Device to prefer. Default: try to auto-detect from platform (CUDA, Metal)')
    parser.add_argument('--output-dir', default='output', help='Output directory to save the results')
    args = parser.parse_args()
    os.makedirs(args.output_dir, exist_ok=True)
    mesh_path = args.mesh
    mesh = o3d.io.read_triangle_mesh(mesh_path)
    out_base = os.path.join(args.output_dir, os.path.basename(mesh_path).split('.')[0])
    tmesh = text2texture(mesh, args.desc, args.steps, os.path.join(os.path.dirname(__file__), 'depth_txt2img.py'), args.image_model, args.device, out_base)
    print('writing new mesh to', mesh_path)
    write_mesh(mesh_path.replace('.obj', '-tex'), tmesh)